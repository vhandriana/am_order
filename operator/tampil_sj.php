<div align="center">
    <h1><label class="label label-success">Data Surat Jalan</label></h1>
    <br>
    <button class="btn btn-primary btn-large" data-toggle="modal" data-target="#tambahModal">
        <i class="glyphicon glyphicon-plus-sign"></i> Tambah Surat Jalan
    </button>
</div>
<br>
    ...
    if (mysql_num_rows($result)) {
        ...
        while ($row = mysql_fetch_array($result)) {
            ...
            <tr>
                ...
                <td><?php echo "<a class='btn btn-info btn-sm' href='admin.php?view=tampil_ubah_pelanggan&idPel=" . $row['idPel'] . "'><i class='glyphicon glyphicon-edit'></i></a> | 
                    <a class='btn btn-danger btn-sm' href='admin.php?view=aksi_hapus_pelanggan&idPel=" . $row['idPel'] . "' onclick='return confirm(&quot;Apakah anda yakin akan menghapus data pelanggan tersebut?&quot;)'><i class='glyphicon glyphicon-trash'></i></a>";
            ?></td>
        </tr>
        <?php
        $no ++;
...
<script type="text/javascript">
    $(document).ready(function () {
        $("button#submit").click(function () {
            $.ajax({
                type: "POST",
                url: "admin/aksi_tambah_pelanggan.php",
                data: $('form#tambah_pelanggan').serialize(),
                success: function (msg) {
                    $("#tambahModal").modal('hide')
                    location.href = 'admin.php?view=tampil_pelanggan';
                    ;
                },
                error: function () {
